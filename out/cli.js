...
if (require.main === module) {
    (() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        program
            .arguments('<url>')
            .description('Humble - HTML page bundler.', {
        ...
        program
            // General options.
            .option('-o --out <type>', 'The path of the file that the HTML archive will be written to.')
            .option('-s --silent', 'Silences all of the console output.')
            .option('-c --check-dependencies', 'Checks which dependencies used by Humble are available on the '
            + 'platform.')
            .option('--use-title', 'Uses the title of the document as the output file name.')
            ...
            + 'HTML page.')
            .option('--dom-purify', 'Sanitizes the HTML using the DOM Purify library.')
            // Format conversion options.
            .option('--markdown', 'Converts the HTML to Markdown format.')
            ...
            .option('--beautify', 'Beautifies the HTML page. Runs `--beautify-html`, '
            ...
            // Readability options.
            .option('--readability-mode', 'Converts the document into readability mode using Mozilla\'s '
            + 'readability library.')
            // Injection options.
            ...
            .option('--socks-proxy <type>', 'Use the provided socks5 proxy as a transport.')
            ...
            .option('--sanitize', 'A prebuilt set of options creating safe HTML page archives by '
            ...
            + 'frames.')
            .option('--compatibility', 'A prebuilt set of options that adds polyfills to the HTML archive and '
            + 'convert resources in the page into formats compatible with older '
            + 'browsers.')
            .option('--simple-minification', 'A prebuilt set of options for losslessly minifying the HTML archive.')
            .option('--advanced-minification', 'A prebuilt set of options for aggressive minification. May break '
            + 'page functionality.')
            .option('--extreme-minification', 'A prebuilt set of options for performing as many optimizations to '
        ...
        if (options.checkDependencies === true) {
            index_1.checkHumbleDependencies();
            process.exit();
        }
        // Creating Humble options from the passed options.
        const humbleOptions = Object.assign({ url: args[0] }, options);
        // Creating a Humble instance and running the archive process.
        const humble = new index_1.Humble(humbleOptions);
        yield humble.archive();
    }))();
}
