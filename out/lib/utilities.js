 ...
 */
function createPathSafeFileName(fileName) {
    // Creating a regular expression to remove path unsafe characters from a string.
    const pathUnsafeReplacementRegex = /[\\/?%*:|"'<>,;=]/g;
    // Removing path unsafe characters and returning the path-safe string.
    const pathSafeFileName = fileName.replace(pathUnsafeReplacementRegex, '');
    return pathSafeFileName;
}
exports.createPathSafeFileName = createPathSafeFileName;
/**
 ...
 * @returns a path-safe file name.
 */
function createFileNameFromTitle(document) {
    // Getting the title from the document.
    const title = document.title;
    // Returning a generic `archive.html` title if the title is empty.
    if (title === '') {
        return 'archive.html';
    }
    // Removing path unsafe characters from the title.
    const pathSafeTitle = createPathSafeFileName(title);
    // Converting the title to snake case.
    const pathSafeSnakeCaseTitle = voca.snakeCase(pathSafeTitle);
    // Adding a `.html` extension to the file name.
    const fileName = `${pathSafeSnakeCaseTitle}.html`;
    return fileName;
}
exports.createFileNameFromTitle = createFileNameFromTitle;
